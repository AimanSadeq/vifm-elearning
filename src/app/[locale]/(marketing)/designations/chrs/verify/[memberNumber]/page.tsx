...
import { Award, Star, CheckCircle2, AlertTriangle, XCircle, Shield, ArrowLeft, Calendar, Building2, Briefcase } from "lucide-react";
...
export default function CHRSVerifyPage() {
  ...
  useEffect(() => {
    async function verify() {
      ...
      const { data: holder } = await supabase
        ...
        .select(
          `member_number, certified_at, status, registry_company, registry_company_ar, registry_title, registry_title_ar, show_in_registry,
          tier:designation_tiers!designation_holders_tier_id_fkey(name, name_ar, slug),
          designation:designations!designation_holders_designation_id_fkey(name, name_ar, abbreviation),
  ...
  if (isLoading) return <div className="flex min-h-[50vh] items-center justify-center"><LoadingSpinner size="lg" /></div>;

  if (notFound) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        <XCircle className="h-16 w-16 text-destructive/50" />
        <h2 className="mt-4 text-xl font-semibold">{locale === "ar" ? "لم يتم العثور على الشهادة" : "Credential Not Found"}</h2>
        <p className="mt-2 text-muted-foreground max-w-md">
          {locale === "ar"
            ? `لم يتم العثور على أي شهادة مرتبطة برقم العضوية "${memberNumber}".`
        ...
        <Link href={`/${locale}/designations/chrs/registry`}>
          <Button className="mt-6"><ArrowLeft className="mr-2 h-4 w-4" />{locale === "ar" ? "العودة إلى السجل" : "Back to Registry"}</Button>
        </Link>
      </div>
    );
  }
  ...
  const statusConfig = {
    active: { icon: CheckCircle2, label: locale === "ar" ? "نشط ومعتمد" : "Active & Verified", color: "text-success", bg: "bg-success/10", border: "border-success/30" },
  ...
  return (
    <div className="mx-auto max-w-lg space-y-6 pb-16">
      <Link href={`/${locale}/designations/chrs/registry`} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
        <ArrowLeft className="h-4 w-4" />{locale === "ar" ? "العودة إلى السجل" : "Back to Registry"}
      </Link>
          ...
          <div className="text-center">
            ...
            {isFoundingMember && (
              <span className="mt-2 inline-flex items-center gap-1 rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-800">
                <Star className="h-3 w-3" />{data.tierName}
              </span>
            )}
          </div>
          <div className="space-y-3 rounded-lg bg-muted/50 p-4">
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground flex items-center gap-2"><Shield className="h-4 w-4" />{locale === "ar" ? "رقم العضوية" : "Member Number"}</span>
              <span className="font-mono font-semibold">{data.memberNumber}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground flex items-center gap-2"><Calendar className="h-4 w-4" />{locale === "ar" ? "تاريخ الاعتماد" : "Certified Since"}</span>
              <span className="font-semibold">{formatDate(data.certifiedAt, locale)}</span>
            </div>
            {data.company && (
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground flex items-center gap-2"><Building2 className="h-4 w-4" />{locale === "ar" ? "الشركة" : "Company"}</span>
                <span className="font-semibold">{data.company}</span>
              </div>
            )}
            {data.jobTitle && (
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground flex items-center gap-2"><Briefcase className="h-4 w-4" />{locale === "ar" ? "المسمى الوظيفي" : "Job Title"}</span>
                <span className="font-semibold">{data.jobTitle}</span>
              </div>
            )}
          </div>
          <div className="text-center border-t pt-4">
            <p className="text-xs text-muted-foreground">{locale === "ar" ? "صادرة عن" : "Issued by"}</p>
            <p className="font-semibold text-sm mt-1">Virginia Institute of Finance and Management (VIFM)</p>
          </div>
        </CardContent>
      </Card>
    </div>
