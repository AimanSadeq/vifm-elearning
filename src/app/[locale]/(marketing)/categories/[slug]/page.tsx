...
export default function CategoryPage() {
  ...
  const [siblings, setSiblings] = useState<
    Array<{ id: string; name: string; name_ar: string | null; slug: string; color: string | null; count: number }>
  >([]);

  const [facetCounts, setFacetCounts] = useState<{
    difficulties: Record<string, number>;
    freeCount: number;
  }>({ difficulties: {}, freeCount: 0 });
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      // Siblings: every other active category and its published-course count
      const { data: allCats } = await supabase
        .from("categories")
        .select("id, name, name_ar, slug, color")
        .eq("is_active", true);

      if (!cancelled && allCats) {
        const withCounts = await Promise.all(
          allCats
            .filter((c) => c.id !== catData.id)
            .map(async (c) => {
      ...
      // Facet counts for this category's published courses
      ...
      if (!cancelled && facetRows) {
        ...
        setFacetCounts({ difficulties, freeCount });
      }
    }
  ...
  const availableDifficulties = ["beginner", "intermediate", "advanced", "expert"].filter(
    (d) => (facetCounts.difficulties[d] ?? 0) > 0
  );
    ...
    <div>
        ...
        <div className="container relative mx-auto px-4 py-10 md:py-14">
          ...
          <div className="mt-6 flex flex-col md:flex-row md:items-end md:justify-between gap-6">
            <div className="max-w-2xl">
              <div
                className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/5 px-3 py-1 text-xs font-medium tracking-wide backdrop-blur"
              >
            ...
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-white/85">
              <Stat
                ...
                value={totalCount > 0 ? totalCount : facetCounts.freeCount + Object.values(facetCounts.difficulties).reduce((a, b) => a + b, 0)}
                label={locale === "ar" ? "دورة" : "Courses"}
              />
              {facetCounts.freeCount > 0 && (
                <>
                  <span className="hidden sm:block h-4 w-px bg-white/20" />
                  <Stat
                    icon={<Sparkles className="h-4 w-4" />}
                    value={facetCounts.freeCount}
      ...
      <div className="container mx-auto px-4 py-10">
        ...
        <div className="mb-6 rounded-2xl border bg-card p-4 shadow-card space-y-4">
          ...
          {(availableDifficulties.length > 0 || facetCounts.freeCount > 0) && (
            <div className="pt-1 border-t">
              <div className="pt-3">
                <CourseFilters
                  ...
                  difficultyCounts={facetCounts.difficulties}
                  freeCount={facetCounts.freeCount}
                />
              </div>
            </div>
          )}
