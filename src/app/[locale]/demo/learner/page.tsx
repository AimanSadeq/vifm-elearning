...
export default async function LearnerDemoDashboard({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  return (
    <DemoShell role="learner" locale={locale} activePath="">
      <div className="mb-8">
      ...
      <div>
        <h2 className="mb-4 flex items-center gap-2 text-xl font-semibold"><Target className="h-5 w-5 text-blue-600" /> Continue Learning</h2>
          ...
          {enrolledCourses.map((c, i) => (
            <div key={i} className="overflow-hidden rounded-xl border bg-card transition-shadow hover:shadow-md">
              ...
              <div className="p-4">
                ...
                <div className="mb-2 flex justify-between text-xs"><span className="text-muted-foreground">{c.lessonsDone} of {c.totalLessons} lessons</span><span className="font-semibold">{c.progress}%</span></div>
                <div className="mb-3 h-1.5 overflow-hidden rounded-full bg-muted"><div className={`h-full rounded-full ${c.progress === 100 ? "bg-emerald-500" : "bg-blue-500"}`} style={{ width: `${c.progress}%` }} /></div>
                <button className="flex w-full items-center justify-center gap-2 rounded-md bg-blue-600 py-2 text-sm font-semibold text-white hover:bg-blue-700"><PlayCircle className="h-4 w-4" />{c.progress === 100 ? "Review Course" : "Continue"}</button>
              </div>
            </div>
          ))}
        </div>
