import type { KeyPointsSlide } from '../../types';
...
export function renderKeyPoints(data: KeyPointsSlide): string {
  const cards = data.points.map((p, i) => {
    const delay = (0.3 + i * 0.12).toFixed(2);
    return `<div class="s1card" style="opacity:0;animation:fsl .5s ${delay}s ease forwards;">
      <div class="s1icon">${renderIcon(p.icon, 'icon-md')}</div>
      <div>
        <div class="s1ctitle">${esc(p.title)}</div>
        <div class="s1cdesc">${esc(p.description)}</div>
      </div>
    </div>`;
  }).join('\n      ');
  ...
  <div class="s1content">
    <div class="s1left" style="opacity:0;animation:fsr .6s .15s ease forwards;">
